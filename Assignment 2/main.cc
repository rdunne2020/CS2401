...
int menu(); //function that lists out selectable things, return int
 int menu2(); //function that lists out selectable things, return int

int main(){

    Friend myfriend; //Friend Object (will call friend constructor)
    FBFriends myfb; //FBFriend Object (will call the FBFriend constructor)
    string friendname; //for searching a friend
    int choice,choice2; //for inputting a numeric choice for bringing up menus
    string username, filename; //for opening/creating files
    ifstream fin; //opens a file to read in from
    ofstream fout; //opens a file to output to
    bool cutout; //IDK what this is doing
  ...
  //cout << "test" << endl; I added this dumb shit
    if(!fin.fail())
	myfb.load(fin);  //calls load function from fbfriends.h, passes in an ifstream (which is an istream still) is getting called because there's no user file
  //cout << "failure" << endl; I added this dumb shit
	fin.close();
	choice = 0;
	choice2 = 0;
	cutout = false;
  FBFriends original(myfb); //Calls copy constructor and passes myfb in
	...
	while(choice != 9)
  {
      ...
      {
		case 1:	cin>>myfriend;
		       	myfb.start(); //calls the FBFriends function start
			      myfb.insert(myfriend); //inputs my friend into the list of names and birthdays
			break;
