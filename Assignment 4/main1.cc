...
// moving the iterator to the third element and putting 89 before that
   ...
   for(int i = 0; i<3; ++it1,++i)
	;
   tmp =89;
   lis1.insert_before(it1, tmp); //can't add before and after
   //lis1.insert_after(it1, tmp);
   for(it1=lis1.begin(); it1 != lis1.end(); ++it1)
        cout<<*it1<<endl;
   cout<<endl<<endl;
   for(it1 = lis1.r_begin(); it1 != lis1.r_end(); --it1)
        cout<<*it1 << " original --it1"<<endl;
...
for(int i = 0; i<lis2.size()/2; --it1,++i)
    ;
cout << *it1 << " where it's being inserted\n"; //it1 is  89, who knows why
lis1.insert_after(it1, tmp);
for(it1=lis2.begin(); it1 != lis2.end(); ++it1)
     cout<<*it1 << " ins after"<<endl;
cout<<endl<<endl;
...
// testing copy constructor and assignment operator
 //works except it copies in a random 256
{
   dlist<int> copy(lis1);
   lis1 = lis2;
   for(it1=lis1.begin(); it1 != lis1.end(); ++it1)
   ...
   for(it1 = lis1.r_begin(); it1 != lis1.r_end(); --it1)
        cout<<*it1<<endl;
   cout<<endl<<endl;
   for(it1=copy.begin(); it1 != copy.end(); ++it1) //random 256 getting in
        cout<<*it1<<endl;
   cout<<endl<<endl;
